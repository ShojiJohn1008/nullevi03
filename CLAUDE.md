...
python3 gcal.py tasks                                 # Google ToDo 未完了一覧
python3 gcal.py addtask "牛乳を買う" 2026-07-10        # ToDo 追加 (期限は任意)
python3 gcal.py donetask <タスクID>                   # ToDo 完了
```
...
- **Google ToDo (Tasks)** の `tasks`/`addtask`/`donetask` を使うには、Apps Script エディタの「サービス」で「Google Tasks API」を追加してから再デプロイする必要がある (`gas/README.md` 参照)。朝のブリーフィング (`briefing.sh` → `prompts/briefing.md`) は毎朝この `tasks` と当日の `list` を呼んで「今日の予定」「残タスク」を読み上げる。そのため `briefing.sh` は `.env` の `CAL_*` を export している (未設定ならブリーフィング側がその項目を飛ばす)。「TODO に追加して」と会話で頼まれたら `gcal.py addtask` で Google ToDo に入れる (従来の `memory/todo.md` はメモ用に併存)。
...
### 個人カレンダーと共有カレンダーの使い分け

予定ごとに書き込み先を選べる (`gcal.py` の `--cal <名前 or ID>`)。基本の運用:

- **仮予定・仮押さえ (`hold`) は個人カレンダー** (既定、`--cal` なし)。まだ他人に見せない段階のものはここ。
- **確定したら共有カレンダーに `create`** (`--cal <共有カレンダー名>`) し、**個人の【仮】を `delete`** する。
  例) 定例が確定 → `gcal.py create "定例MTG" <開始> <終了> --cal チーム` してから、個人の仮押さえを `gcal.py delete <id>`。
- どの共有カレンダーに上げるか不明なときは Telegram で確認する。共有カレンダーへ書くにはそのカレンダーの変更権限が必要。
- 共有カレンダーに予定を出すとそれを共有している人に見えるので、**個人的な内容をうっかり共有カレンダーに入れない**よう、宛先は毎回意識する。
